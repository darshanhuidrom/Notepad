...
public class ScanBarcodeActivity extends AppCompatActivity {
    ...
    private void initViews() {
        ...
        barcodeValue = findViewById(R.id.txtBarcodeValue);
        btnAction.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=null;
                if(intentData.length()>0&&isEmail){
                     intent= new Intent(ScanBarcodeActivity.this,EmailActivity.class)
                             .putExtra("email_address",intentData);
                }
                else {
                    intent= new Intent(Intent.ACTION_VIEW, Uri.parse(intentData));
                }
                    ...
                    barcodeValue.post(new Runnable() {
                        ...
                        public void run() {
                            if (barcodes.valueAt(0).email != null) {
                                barcodeValue.removeCallbacks(null);
